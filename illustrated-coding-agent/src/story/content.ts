...
export const INTRO_CONTENT: ContentBlock[] = [
  ...
  // ── Toy Example: Rename a Variable ────────────────────────────────
  {
    ...
    body: 'The concepts you just learned — model, harness, tools, permissions — are big. Before we watch a full agent build a landing page, let\'s trace one tiny task from start to finish: rename a variable. Five steps. One file. The same machine you\'ll watch in the Flight Recorder, stripped to its essentials.',
    lensMode: 'product',
    ...
    misconception: {
      ...
      whyItMatters: 'If you can trace a 5-step variable rename, you can trace a 50-step feature build. The mechanism doesn\'t change — only the number of iterations.',
    },
  ...
  },
  {
    ...
    chapterId: 'toy-example',
    sceneId: 'toy-example-rename',
    heading: 'Step 2: Identify the target',
    body: 'The model reads the observation and reasons: the variable userCount should be renamed to activeUserCount. This is the model\'s job — pure reasoning about the code it just saw. Note what didn\'t happen: no tool got called. The model proposed the next action; the harness hasn\'t executed anything yet.',
    lensMode: 'product',
  },
  {
    id: 'toy-step-write',
    chapterId: 'toy-example',
    sceneId: 'toy-example-rename',
    heading: 'Step 3: Write the change',
    body: 'The model emits a write_file request with the diff: s/userCount/activeUserCount/. The harness validates the request, checks sandbox rules (is this file in the allowed set?), and dispatches the write. The file is now updated. This is the act phase of the loop — the model\'s intent turned into a real filesystem change.',
    lensMode: 'product',
  },
  {
    id: 'toy-step-verify',
    chapterId: 'toy-example',
    sceneId: 'toy-example-rename',
    heading: 'Step 4: Run the tests',
    body: 'The model emits run_tests. The harness executes the test suite. The result comes back: all tests pass. The rename is correct. This is the verify phase — without it, the agent has no way to know whether its change actually worked. Output without verification is speculation.',
    lensMode: 'product',
  },
